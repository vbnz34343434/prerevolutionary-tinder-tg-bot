...
@Component
public class NavigateRightHandler implements CommandHandler {
    ...
    public NavigateRightHandler(UserBotState userBotState, SearchPairService searchPairService, PairsNavigationService pairsNavigationService, SendMessageMakerService sendMessageMakerService, @Lazy TelegramTinderBot bot) {
    ...
    public BotApiMethod<?> handle(long userId, String chatId, String commandText) {
        ...
        switch (state) {
            case IN_SEARCH:
                searchPairService.likeCurrentUser(userId);
                userForShow = searchPairService.searchNext(userId, false);
                bot.executeMessage(
                ...
                break;
            case VIEW_PAIRS:
                userForShow = pairsNavigationService.navigate(userId, "next");
                bot.executeMessage(
                        sendMessageMakerService.createPhotoMessage(
                                chatId,
